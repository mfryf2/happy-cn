...
本仓库在 [slopus/happy](https://github.com/slopus/happy) 基础上增加了以下功能：

### 🖼️ 图片上传支持（Web 端）
在网页聊天框中直接粘贴（Ctrl+V / Cmd+V）或拖拽图片，图片会上传到服务器并发送给 AI 进行分析。支持多图同时上传。

### 🇨🇳 中国大陆适配
- 支持通过 `HAPPY_CLAUDE_PATH` 环境变量指定任意 Claude Code 可执行文件（包括公司定制版）
- 默认服务器地址指向国内（`happy.mingbaibao.com`），无需翻墙
- 支持带 `#!/usr/bin/env node` shebang 的定制版启动方式

### 🔧 权限按钮交互优化
修复了权限确认弹窗中按钮高亮状态不准确的问题（服务端不总是返回 `allowedTools` 字段，改为本地状态追踪用户点击意图）。

### 🚀 部署运维脚本
提供一键部署和更新脚本，方便自建服务器用户维护：
- `scripts/update-local.sh` — 同步上游并重新编译
- `scripts/update-server.sh` — 更新服务器后端和前端
...
本仓库基于 [slopus/happy](https://github.com/slopus/happy)，在完全兼容上游的基础上做了以下改动：

| 类别 | 改动内容 |
|---|---|
| **图片上传** | `AgentInput.tsx` 支持 Web 端粘贴/拖拽图片；`apiUpload.ts` 图片上传 API；服务端 `uploadRoutes.ts` 上传接口 |
| **中国适配** | `detectCLI.ts` + `claudeLocalLauncher.ts` 支持 `HAPPY_CLAUDE_PATH` 自定义路径；shebang 脚本启动方式 |
| **权限修复** | `PermissionFooter.tsx` 本地 `selectedAction` 状态，解决权限按钮高亮不准问题 |
| **部署脚本** | `scripts/update-local.sh`、`scripts/update-server.sh`、`scripts/deploy.sh` |
| **文档** | `README.zh.md`、`SELF_HOSTED_GUIDE.md`、`FORK_CHANGES.md` |

详细改动记录见 [FORK_CHANGES.md](./FORK_CHANGES.md)。与上游完全向后兼容，可随时同步官方更新。
...
脚本自动完成：拉取上游 → 合并 → 重新编译 → 推送到你的 fork。
