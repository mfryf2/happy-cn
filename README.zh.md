...
[🌐 **Web App 体验地址**](https://happy.mingbaibao.com) · [📖 **English README**](./README.md) · [💬 **提 Issue**](https://github.com/slopus/happy/issues)
...
### 场景二：自定义内部版
使用公司内部定制版 Claude Code（如腾讯 `claude-internal`、字节 `claude-xxx` 等带 `#!/usr/bin/env node` shebang 的可执行脚本）的用户。

→ [快速开始 — 场景二](#场景二自定义内部版-claude-code-1)
...
1. **Mac/Linux 本地环境**，已能正常运行 `claude`（或你的自定义版本）
...
## 场景二：自定义内部版 Claude Code

### 适用人群
- 使用公司内部定制的 Claude Code 版本
- 可执行文件带有 `#!/usr/bin/env node` shebang（如腾讯 `claude-internal`）

### 与场景一的区别

官方 happy 使用 `import()` 加载 Claude，但内部版是**独立可执行脚本**，必须用 `spawn` 方式启动。本仓库已包含此修复（`claude_version_utils.cjs` 中的 shebang 检测）。
...
```bash
which claude-internal   # 或者你公司内部版的命令名
# 例如：/opt/homebrew/bin/claude-internal
```
...
```bash
# 在 ~/.zshrc 中添加
export HAPPY_CLAUDE_PATH="/opt/homebrew/bin/claude-internal"  # 改为你的实际路径
export HAPPY_SERVER_URL="https://happy.mingbaibao.com"
export HAPPY_WEBAPP_URL="https://happy.mingbaibao.com"
alias happy="node /path/to/happy/packages/happy-cli/dist/index.mjs"
```
...
```bash
happy   # 启动你的内部版 Claude Code，支持远程控制
```

### 注意事项

- `happy` 命令完全替代你原来的 `claude-internal` 命令，所有参数透传
- 原来的 `claude-internal` 命令**不受影响**，两者互不干扰
- 不支持 `happy codex`（若内部版禁用了 `app-server` 子命令）
...
| 文件 | 改动内容 |
|---|---|
| `packages/happy-cli/scripts/claude_version_utils.cjs` | 增加 shebang 检测，支持内部定制版（带 `#!/usr/bin/env node` 的独立脚本） |
| `README.zh.md` | 新增（本文件） |
...
happy 支持 Windows，但 shebang 检测修复仅针对 macOS/Linux。Windows 上的内部版需要额外适配。
